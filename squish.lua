...
function GetModuleName(name)
    map[name] = map[name] or (BASE_NAME .. "/" .. RandomName(12))

    return map[name]
end

function GetRequireString(module)
    local function lol(name)
        return '"' .. name .. '"'
    end

    return lol(module.og_name), lol(module.og_name)
end
...
if opts.very_verbose then
    opts.verbose = true;
end
if opts.very_quiet then
    opts.quiet = true;
end

local noprint = function()
end
...
if opts.verbose or opts.very_verbose then
    print_verbose = print;
end
if opts.very_verbose then
    print_debug = print;
...
function Module(name)
    ...
    modules[GetModuleName(name)] = modules[i];
...
do
    ...
    for i, module in ipairs(modules) do
        if not module.path then
            module.path = resolve_module(module.og_name, package_path);
            if not module.path then
...
local fin, err = io.open(base_path .. "map.txt", "w")

print_verbose("Packing modules...");
for _, module in ipairs(modules) do
    local modulename, path = module.og_name, module.path;
    ...
    if data then
        ...
        local original_str, replace_str = GetRequireString(module)
        fin:write(original_str .. " " .. replace_str .. "\n")

        f:write(data);
        f:write(" end)\n");

        -- f:write('require "', modulename, '"\n');
        ...
        print_err("Couldn't pack module '" .. modulename .. "': " ..
                      (err or "unknown error... path to module file correct?"));
        os.exit(1);
    end
end
...
if #resources > 0 then
    ...
    for _, resource in ipairs(resources) do
        ...
        local maxequals = 0;
        data:gsub("(=+)", function(equals_string)
            maxequals = math.max(maxequals, #equals_string);
        end);
